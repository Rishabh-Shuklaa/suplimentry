...
def imresize(im,scale,opt):
    im = torch2uint8(im)
    im = imresize_in(im, scale_factor=scale, opt = opt)
    im = np2torch(im,opt)
    return im

def imresize_to_shape(im,output_shape,opt):
    im = torch2uint8(im)
    im = imresize_in(im, output_shape=output_shape, opt = opt)
    im = np2torch(im,opt)
    return im


def imresize_in(im, scale_factor=None, output_shape=None, kernel=None, antialiasing=True, kernel_shift_flag=False, opt = None):
    ...
    for dim in sorted_dims:
        ...
        weights, field_of_view = contributions(im.shape[dim], output_shape[dim], scale_factor[dim],
                                                       method, kernel_width, antialiasing, low = 0, high = 0)
...
def contributions(in_length, out_length, scale, kernel, kernel_width, antialiasing, low = 0, high = 0):
