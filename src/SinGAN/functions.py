...
def calc_gradient_penalty(netD, real_data, fake_data, LAMBDA, device, mask = None):
    ...
    if mask is not None:
        disc_interpolates = netD(mask*interpolates)
    else:
...
def read_image(opt):
    if opt.mode == 'inpainting':
        x = img.imread('%s/%s' % (opt.input_dir, opt.ref_name))
    else:
...
def dilate_mask(mask,opt, it=0):
    ...
    if it > 0:
        plt.imsave('%s/%s_mask_dilated_%d.png' % (opt.ref_dir, opt.input_name[:-4], it), convert_image_np(mask), vmin=0, vmax=1)
    else:
